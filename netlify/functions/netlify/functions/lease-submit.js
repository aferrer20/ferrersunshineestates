...
exports.handler = async (event) => {
  ...
  if (!KEY || !FROM || !TO) {
    return err(500, 'Mail not configured. Set RESEND_API_KEY, MAIL_FROM and MAIL_TO in Netlify.');
  }

  let d;
  try { d = JSON.parse(event.body || '{}'); } catch (e) { return err(400, 'Bad JSON'); }
  ...
  const payUrl = d.payUrl && d.payUrl.startsWith('http') ? d.payUrl : '';

  if (!org.email || !org.company || !org.signer) {
    return err(400, 'Missing required fields: org.email, org.company, org.signer');
  }
...
<p style="font-size:14px;line-height:1.7;color:#6B5C4E;margin:0">Questions — reply to this email or call (727) 504-9949.</p>`
  ...
  try {
    ...
    return {
      statusCode: 502,
      headers: { ...CORS, 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: 'Owner notification failed', detail: String(e.message) }),
    };
  }
